...
from player.blue.rule_player_training import RulePlayerTraining
...
class DogFightStat(AgentStat):
    ...
    def summarise(self):
        result = super(DogFightStat, self).summarise()
        print('result', result)
        return result


class DogFightPlayer(AgentInterface):
    ...
    def __init_variables(self):
        ...
        self.rule_player = RulePlayerTraining(self.side)
        self.__agent_stat = DogFightStat()
    ...
    def transform_action2command(self, action, raw_obs):
        ...
        """
        cmds = []
        cmds.extend(self.rule_player.step(raw_obs[self.side]))
        print('cmd num:', len(cmds))

        command, valid_actions = self._make_commands(action)
        # if self.rule_player.rl_flag:
